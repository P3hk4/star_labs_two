...
public class LinkedListTabulatedFunction extends AbstractTabulatedFunction implements Cloneable, Serializable {
    ...
    public LinkedListTabulatedFunction(MathFunction source, double xFrom, double xTo, int count) {
        if (count < 2) throw new IllegalArgumentException("длина меньше минимальной");
            ...
            if (xFrom != xTo) {
                double gap = (xTo - xFrom) / (count - 1);
                for (int i = 0; i < count; i++) {
                    addNode(xFrom + i * gap, source.apply(xFrom + i * gap));
                }
            } else {
                for (int i = 0; i < count; i++) {
    ...
    protected int floorIndexOfX(double x) {
        if (head.x > x)  throw new IllegalArgumentException("значение выходит за рамки допустимых значения");
        else {
            Node elem = head;
            int i = 1;
            do {
    ...
    protected double interpolate(double x, int floorIndex) {
        ...
        if (leftX > x || rightX < x) throw new InterpolationException("X находится вне допустимого интервала значений");
