...
public class ArrayTabulatedFunction extends AbstractTabulatedFunction implements Cloneable{
    ...
    public double getX(int index) {
        if(index < 0 || index >= this.count)  throw new IllegalArgumentException("индекс выходит за рамки допустимых значения");
    ...
    public int indexOfX(double x) {                 //Поиск индекса значения использует бинарный поиск в массиве
        if ((x < this.leftBound()) || (x > this.rightBound()))  throw new IllegalArgumentException("индекс выходит за рамки допустимых значения");              //Выход из поиска при искомом значении вне границ значений массива
    ...
    public int indexOfY(double y) {               //Поиск индекса значения использует бинарный поиск в массиве (Аналогично поиску индекса Х по значению)
        if ((y < this.getY(0)) || (y > this.getY(this.count-1)))  throw new IllegalArgumentException("индекс выходит за рамки допустимых значения");
